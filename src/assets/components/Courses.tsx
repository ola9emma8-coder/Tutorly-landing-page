...
export default function Courses() {
  ...
  const CardContent = ({ course }: { course: (typeof courses)[0] }) => (
    <>
      <img
        src={course.image}
        className="rounded-lg w-full h-44 block m-auto object-cover mt-2"
      />
      <div className="flex flex-row gap-3 xs:gap-4 sm:gap-5 justify-center mt-2 items-center flex-wrap px-1">
      ...
      <div className="flex flex-row gap-1 ml-2">
        <p className="font-bold text-sm sm:text-md text-black">
          {course.price.new}
        </p>
      ...
      <button
        className="border-gray-500 transition duration-700 ease-in-out
        hover:text-white hover:bg-black border w-[90%] h-7 text-xs sm:text-sm mt-2
        rounded-l-3xl text-center font-bold cursor-pointer rounded-r-3xl block mx-auto"
      >
        {course.buttonText}
      </button>
    </>
  ...
  return (
    <div className="py-8">
      <motion.h1
        ...
        className="text-2xl font-bold md:text-3xl lg:text-4xl text-black text-center"
      >
        <span className="block">Our Featured Courses</span>
      </motion.h1>
        ...
        <div className="flex flex-col gap-5">
          {courses.map((course, index) => (
            <motion.div
              ...
              transition={{ duration: 0.5, ease: "easeIn", delay: index * 0.15 }}
              viewport={{ amount: 0.2, once: true }}
              className="rounded-lg bg-white shadow-sm border border-gray-100 pb-3 w-full max-w-sm mx-auto"
            >
      ...
      {/* TABLET sm–md (sm to below lg): 2-column grid, 3rd card centered */}
      <div className="hidden sm:block lg:hidden px-4 sm:px-6 md:px-10 mt-10">
        <div className="grid grid-cols-2 gap-5 max-w-2xl mx-auto">
          {courses.map((course, index) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.55, ease: "easeIn", delay: index * 0.2 }}
              viewport={{ amount: 0.2, once: true }}
              className={`rounded-lg bg-white shadow-sm border border-gray-100 pb-3
                ${index === 2 ? "col-span-2 max-w-sm mx-auto w-full" : ""}`}
            >
              <CardContent course={course} />
            </motion.div>
          ))}
      ...
      {/* DESKTOP lg+: your original exact design */}
      <div className="hidden lg:flex lg:justify-center lg:items-center gap-5 mt-10">
        {courses.map((course, index) => (
          <motion.div
            key={index}
            ...
            viewport={{ amount: 0.3, once: true }}
            className="rounded-lg bg-white shadow-sm w-70 h-72 border-white"
          >
            <img
              src={course.image}
            ...
            <button
              className="border-gray-500 transition duration-700 ease-in-out
              hover:text-white hover:bg-black border w-65 h-6 text-sm mt-2
              rounded-l-3xl text-center font-bold cursor-pointer rounded-r-3xl block m-auto"
            >
              {course.buttonText}
            </button>
          </motion.div>
