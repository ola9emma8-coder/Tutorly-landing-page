...
export default function Tutorly() {
  ...
  // Mobile/tablet card — fixed height removed, text sized to fit
  const Card = ({ item }: { item: (typeof ips)[0] }) => (
    <div className="flex flex-col gap-2 px-3 pt-3 pb-3 h-full">
      <item.icon className="w-3 h-3 font-black shrink-0" />
      <div className="flex flex-col gap-0.5">
        <h2 className="text-black text-sm sm:text-base font-bold leading-snug">
          {item.heading}
        </h2>
        <p
          style={{ wordSpacing: "-1px" }}
          className="text-black text-[10px] sm:text-xs leading-snug"
        >
          {item.text}
        </p>
      </div>
    ...
    <div className="mt-10">
        ...
        <div className="grid grid-cols-2 gap-3 items-stretch">
          {[...ips, ...ars].map((item, index) => (
            <motion.div
              ...
              viewport={{ amount: 0.3, once: true }}
              className="border-white shadow-xs bg-white rounded-lg h-auto min-h-30"
            >
              <Card item={item} />
            </motion.div>
          ))}
      ...
      <div className="hidden lg:flex flex-row justify-center gap-3 mt-5">
        <div className="flex flex-col gap-3">
          {ips.map((ip, index) => (
            <motion.div
              ...
              key={index}
              className="w-50 h-30 border-white 
            ...
            >
              <div className="flex flex-col gap-3 ml-3 mt-3">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <p style={{ wordSpacing: "-1px" }} className="text-black text-xs">
                    {ip.text}
                  </p>
                </div>
              </div>
        ...
        <motion.img
          ...
          viewport={{ amount: 0.3, once: true }}
          src="/images/tutorly.jpeg"
          className="rounded-lg w-100 object-cover h-60"
        />
